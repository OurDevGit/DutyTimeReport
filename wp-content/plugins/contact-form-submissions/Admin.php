...
class WPCF7SAdmin
{
    ...
    public function filters()
    {
        ...
        if ($post_type == 'wpcf7s') {
            ...
            <select name="selectUser">
               <option value="0"><?php _e('ALL', 'contact-form-submissions'); ?></option>
                <?php foreach ($users as $user) {
                ?>
                    <?php $selected = ($user->ID == $_GET['selectUser']) ? 'selected' : ''; ?>
                    <option value="<?php echo $user->ID; ?>" <?php echo $selected; ?>><?php echo $user->user_login; ?></option>
                <?php
            } ?>
            </select>
                    ...
                    <?php 
                        ...
                        { 
                            if(isset($_GET['selectYear'])){
                                $selected = ($i == $_GET['selectYear']) ? 'selected' : ''; 
                            }else{
    ...
    public function admin_posts($query)
    {
        ...
        if ($query->is_admin && 'wpcf7s' === $post_type && $query->is_main_query()) {
            ...
            if(isset($_GET['selectUser']) && $_GET['selectUser'] != 0){
                $user_id = esc_attr($_GET['selectUser']);
                array_push($array, array(
                        'key'     => 'user_id',
                        'value'    => $user_id,
                        'compare' => '='
            ...
            if(isset($_GET['selectMonth'])){
                ...
                if($_GET['selectMonth'] == 0){
                    $from = date($year."-01-01");
                    $to = date($year."-12-31");
                }else{
                    $from = date($year."-".$_GET['selectMonth']."-01");
                    $to = date($year."-".$_GET['selectMonth']."-31");
                }
                
                array_push($array, array(
                        'key'     => 'wpcf7s_posted-date',
                        'value'    => $from,
                        'compare' => '>='
                    ));
                array_push($array, array(
    ...
    public function set_columns($columns)
    {
        ...
        $columns = array(
            ...
            // 'UserName'      => __('UserName', 'contact-form-submissions'),
            // 'Date'          => __('Date', 'contact-form-submissions'),
            // 'onDutyTime'    => __('onDutyTime', 'contact-form-submissions'),
            // 'offDutyTime'   => __('offDutyTime', 'contact-form-submissions'),
            // 'Duty'          => __('Duty', 'contact-form-submissions'),
            // 'Vehicle'       => __('Vehicle', 'contact-form-submissions'),
            // 'BodyCam'       => __('BodyCam', 'contact-form-submissions'),
            // 'Hours'         => __('Hours', 'contact-form-submissions'),
            // 'Comments'      => __('Comments', 'contact-form-submissions'),
        ...
        if (isset($_GET['wpcf7_contact_form']) && !empty($_GET['wpcf7_contact_form'])) {
            ...
            foreach ($wpcf7s_columns as $meta_key) {
                if($meta_key != 'wpcf7s_posted-partner' && $meta_key != 'wpcf7s_posted-typeName'){
        ...
        // $columns['TotalHours'] = __('TotalHours', 'contact-form-submissions');
    ...
    public function column($column, $post_id)
    {
        ...
        switch ($column) {
            ...
            case 'submission':
                ...
                break;
            default:
                if($column == "wpcf7s_posted-Hours")
                {
                    $total += get_post_meta($post_id,"wpcf7s_posted-Hours" , true);
                }
    ...
    public function get_available_columns($form_id = 0)
    {
        ...
        $columns = $wpdb->get_col("SELECT meta_key FROM $wpdb->postmeta WHERE post_id = $post_id AND meta_key LIKE '%wpcf7s_%' GROUP BY meta_key ORDER BY meta_id");
    ...
    public function extra_tablenav()
    {
        ...
        if ('wpcf7s' === $screen->post_type){
            ...
            <?php
                if($total > 0){
                    echo "<a style='font-size:20px'>TotalHours :".$total."hrs</a>";
                }
        }
    }
